...
function loadManageableTarget(req, res, { blockSelf } = {}) {
...
router.get('/', (req, res) => {
  ...
  const staff = getAllStaff().map(s => ({
    ...s,
    orgCount: getOrgIdsForStaff(s.id).length,
...
router.post('/', express.urlencoded({ extended: true }), verifyCsrf, async (req, res) => {
  ...
  try {
    ...
    return res.status(400).send(/UNIQUE/.test(error.message) ? 'That MTAKPI staff name is already linked to an account' : error.message);
  }
  res.redirect('/staff');
});
...
router.post('/import', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
...
router.post('/:id/deactivate', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
  const target = loadManageableTarget(req, res, { blockSelf: true });
  if (!target) return;
  deactivateStaff(target.id);
  res.redirect('/staff');
});

router.post('/:id/reactivate', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
  const target = loadManageableTarget(req, res);
  if (!target) return;
  reactivateStaff(target.id);
  res.redirect('/staff');
});
...
router.post('/:id/delete', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
  const target = loadManageableTarget(req, res, { blockSelf: true });
  if (!target) return;
...
router.post('/:id/role', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
  const target = getStaffById(req.params.id);
  ...
  updateStaffRole(target.id, role);
  res.redirect('/staff');
});

router.post('/:id/initials', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
  const target = loadManageableTarget(req, res);
  if (!target) return;
  ...
  updateStaffInitials(target.id, initials);
  res.redirect('/staff');
});
...
router.post('/:id/settings-access', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
  const target = loadManageableTarget(req, res);
  if (!target) return;
  setStaffCanManageSettings(target.id, req.body.can_manage_settings === '1');
  res.redirect('/staff');
});
...
router.get('/:id/edit', (req, res) => {
  ...
  const assignedIds = new Set(getOrgIdsForStaff(staff.id));
...
router.post('/:id/access', express.urlencoded({ extended: true }), verifyCsrf, (req, res) => {
  const target = loadManageableTarget(req, res);
  if (!target) return;
  const orgIds = [].concat(req.body.org_ids || []).map(Number).filter(Number.isFinite);
  replaceStaffOrgAccess(target.id, orgIds);
